# TODO: This needs to be compiled with Intel C++ Compiler due to ENABLE_INTEL_OPENMP ? (If openmp is false, it gets the compiler default one)
# TODO: Remove TBB 2.5 when OneTBB released, 
# TODO: Add autoAddDriverRunpath when tests are working
# TODO: Run the tests on the packaging, there is a github issue created
# TODO: Manual test openvino plugins
# TODO: Enable ENABLE_OV_JAX_FRONTEND
{
  lib,
  stdenv,
  fetchFromGitHub,
...
stdenv.mkDerivation rec {
  ...
  cmakeFlags = [
    ...
    # This fails regardless:
    (cmakeBool "ENABLE_OV_JAX_FRONTEND" false) # on true: auto-patchelf could not satisfy dependency libopenvino_jax_frontend.so.2450
  ...
  postInstall = ''
    mkdir -p $out/lib $out/include $out/lib/cmake/openvino

    for lib in $out/runtime/lib/intel64/*; do
      ln -s "$lib" $out/lib/ || cp -a "$lib" $out/lib/
    done

    for hdr in $out/runtime/include/*; do
      ln -s "$hdr" $out/include/ || cp -a "$hdr" $out/include/
    done

    for cmakeFile in $out/runtime/cmake/*; do
      ln -s "$cmakeFile" $out/lib/cmake/openvino/ || cp -a "$cmakeFile" $out/lib/cmake/openvino/
    done
